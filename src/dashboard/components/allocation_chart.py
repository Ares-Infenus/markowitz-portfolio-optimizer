...
def render(selected: list[str]) -> None:
    ...
    with c2:
        view = st.selectbox(
            "View", ["Grouped bars", "Stacked bars", "Donut charts"],
            label_visibility="collapsed",
        )
    with c3:
        st.markdown(
...
def _render_bars(snapshot, methods, ticker_cols, stacked: bool) -> None:
    ...
    fig.add_hline(
        y=eq_w, line_dash="dot", line_color="#2D3748", line_width=1.5,
    ...
    fig.update_layout(
        **_dark_layout(height=440),
        ...
        yaxis=dict(
            ...
            gridcolor="#0D1220", zeroline=False,
        ),
        legend=dict(
...
def _render_donuts(snapshot, methods, ticker_cols) -> None:
    ...
    ticker_colors = [
        ...
        "#6BCB77","#4A9955","#FFD93D","#888888","#FF8C00","#BB6600",
    ]
...
def _add_sector_bands(fig, ticker_cols) -> None:
    ...
    for sector in sectors_seen:
        ...
        fig.add_vrect(
            x0=x0, x1=x1, fillcolor=bg, opacity=0.4,
