...
_EVENTS = [
    {"name": "COVID Crash",      "start": "2020-02-20", "end": "2020-04-01",  "color": "rgba(239,68,68,0.08)"},
    {"name": "Bear Market 2022", "start": "2022-01-01", "end": "2022-12-31",  "color": "rgba(245,158,11,0.07)"},
]
...
def render(selected_methods: list[str]) -> None:
    ...
    for method in df["method"].unique():
        ...
        fig.add_trace(
            go.Scatter(
                x=sub_full["date"], y=sub_full["portfolio_value"],
        ...
        fig.add_trace(
            go.Scatter(
                x=sub_full["date"], y=dd,
                mode="lines", name=label,
                line=dict(color=color, width=1.2, dash=dash),
                fill="tozeroy",
                fillcolor=color.replace(")", ",0.07)").replace("rgb", "rgba") if color.startswith("rgb") else _hex_to_rgba(color, 0.07),
    ...
    for ev in _EVENTS:
        for r in [1, 2]:
            fig.add_vrect(
                x0=ev["start"], x1=ev["end"], fillcolor=ev["color"], line_width=0,
                row=r, col=1,
            )
    ...
    fig.update_yaxes(title_text="Portfolio Value", title_font_size=11, row=1, col=1,
                     gridcolor="#111827", zeroline=False)
    fig.update_yaxes(title_text="Drawdown %", title_font_size=11, row=2, col=1,
                     gridcolor="#111827", ticksuffix="%", zeroline=False)
