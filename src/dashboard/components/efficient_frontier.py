...
def render(selected: list[str]) -> None:
    ...
    if _SPECIALS_PATH.exists():
        ...
        for _, row in specials.iterrows():
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    symbol=s["symbol"], color=s["color"], size=s["size"],
    ...
    if not perf.empty:
        for _, row in perf.iterrows():
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    symbol="star", color=color, size=20,
    ...
    fig.update_layout(
        **_dark_layout(height=480),
        xaxis=dict(
            title="Annualised Volatility", tickformat=".0%",
            gridcolor="#0D1220", zeroline=False, title_font_size=11,
        ),
        yaxis=dict(
            title="Annualised Return", tickformat=".0%",
            gridcolor="#0D1220", zeroline=False, title_font_size=11,
        ),
        legend=dict(
            x=0.01, y=0.01, bgcolor="rgba(0,0,0,0)",
