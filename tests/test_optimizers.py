...
def test_weights_sum_to_one(
    optimizer_instance, synthetic_returns, synthetic_cov, synthetic_expected_returns
):
    """Weights must sum to 1.0 within tolerance for ALL methods."""
    weights = optimizer_instance.optimize(
        synthetic_returns, synthetic_cov, synthetic_expected_returns
    )
    assert (
        abs(weights.sum() - 1.0) < TOL
    ), f"Weights sum={weights.sum():.8f} for {type(optimizer_instance).__name__}"


def test_no_weight_exceeds_max(
    optimizer_instance, synthetic_returns, synthetic_cov, synthetic_expected_returns, settings
):
    ...
    assert (
        weights <= settings.max_weight + TOL
    ).all(), f"Weight exceeds max: {weights.max():.4f} > {settings.max_weight} in {type(optimizer_instance).__name__}"
...
def test_no_weight_below_min(
    ...
    assert (
        weights >= settings.min_weight - TOL
    ).all(), f"Weight below min: {weights.min():.4f} < {settings.min_weight} in {type(optimizer_instance).__name__}"


def test_output_is_series_indexed_by_tickers(
    optimizer_instance, synthetic_returns, synthetic_cov, synthetic_expected_returns, settings
):
...
def test_risk_parity_equal_contributions(
    settings, synthetic_returns, synthetic_cov, synthetic_expected_returns
):
...
def test_cvar_lower_tail_risk(
    ...
    assert (
        cvar_cvar95 <= ew_cvar95 + 0.005
    ), f"CVaR optimizer CVaR ({cvar_cvar95:.4f}) worse than equal weight ({ew_cvar95:.4f})"


def test_black_litterman_top_momentum_higher_weight(
    settings, synthetic_returns, synthetic_cov, synthetic_expected_returns
):
...
def test_efficient_frontier_shape(
    ...
    frontier = optimizer.efficient_frontier(
        synthetic_returns, synthetic_cov, synthetic_expected_returns, n_points=10
    )
